#I modifed Pahikkala et al. (2014) and DeepDTA source codes for cross-val process
...
def build_combined_categorical(FLAGS, NUM_FILTERS, FILTER_LENGTH1, FILTER_LENGTH2):
    ...
    encode_smiles =Conv1D(64, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = Conv1D(64, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = MaxPooling1D(1)(encode_smiles)
    encode_smiles =Conv1D(256, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = Conv1D(256, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = MaxPooling1D(1)(encode_smiles)
    encode_smiles =Conv1D(512, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = Conv1D(512, 4,strides=2, activation='relu',padding="same")(encode_smiles)
    encode_smiles = MaxPooling1D(1)(encode_smiles)
    encode_smiles = GlobalMaxPooling1D()(encode_smiles)
    encode_smiles = Dropout(0.3)(encode_smiles)

    encode_protein = Embedding(input_dim=FLAGS.charsmiset_size+1, output_dim=128, input_length=FLAGS.max_smi_len)(XTinput)
    encode_protein =Conv1D(64, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = Conv1D(64, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = MaxPooling1D(1)(encode_protein)
    encode_protein =Conv1D(256, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = Conv1D(256, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = MaxPooling1D(1)(encode_protein)
    encode_protein =Conv1D(512, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = Conv1D(512, 4,strides=2, activation='relu',padding="same")(encode_protein)
    encode_protein = MaxPooling1D(1)(encode_protein)
...
def experiment(FLAGS, perfmeasure, deepmethod, foldcount=6): #5-fold cross validation + test #default foldcount equals 6
...
def run_regression( FLAGS ):
    ...
    deepmethod = build_combined_categorical ##run function
